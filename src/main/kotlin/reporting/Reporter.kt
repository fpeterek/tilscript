...
class Reporter private constructor(sourceFile: String = "") {
    ...
    private fun formatReport(report: Report): String {
        ...
        val rightAvailable = line.length - report.char
        val rightUnused = max(19 - rightAvailable, 0)
        val leftAvailable = report.char - 1

        val leftMax = 40 + rightUnused
        val takeLeft = max(leftAvailable, leftMax)

        val dropLeft = max(report.char - takeLeft, 0)

        val positionIndicator = "(${report.line}, ${report.char}): "
        val trimmed = line.drop(dropLeft).take(60)

        val pointer = " ".repeat(report.char - dropLeft - 4) + "~~~" + " ^" + when {
            rightUnused > 1 -> " " + "~".repeat(max(rightUnused-1, 3))
            else -> ""
        }
